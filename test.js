...
const multiServerPayload = {
  results: [{
    ...
    scan: { finishedAt: new Date().toISOString(), finalStreams: [
      { server: 'Alpha', kind: 'hls', url: 'https://cdn/1080/index.m3u8', probe: fakeProbe, headers: {} },
      { server: 'Alpha', kind: 'hls', url: 'https://cdn/1080/index.m3u8', probe: fakeProbe, headers: {} },
      { server: 'Premium', kind: 'hls', url: 'https://cdn/1080/index.m3u8', probe: fakeProbe, headers: {} },
...
assert.equal(rows.length, 2, 'one stream per content + server + quality is published');
assert.deepEqual(rows.map((row) => row.server), ['Alpha', 'Premium']);
assert.ok(rows.every((row) => row.quality === '1080p' && row.resolution === '1920x1080'));
...
assert.equal(scanner.is1080ClassResolution('1800x800'), false);
