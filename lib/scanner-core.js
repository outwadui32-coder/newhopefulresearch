...
function checkpointBatchItem(state, canonical, scan, mode) {
  ...
  if (!batch.itemResults) batch.itemResults = {};
  batch.itemResults[canonical] = {
    title: (state.catalog[canonical] && state.catalog[canonical].title) || canonical,
    contentType: contentType(state.catalog[canonical] || { canonicalId: canonical }),
    mode: mode,
    success: normalized.success,
    approvedStreams: normalized.finalStreams.filter(streamIsPublishable).length,
    rejectedStreams: normalized.finalStreams.filter((stream) => !streamIsPublishable(stream)).length
  };
}
...
function completeBatch(state, options) {
  options = options || {};
  const batch = state.activeBatch;
  ...
  if (options.advancePointer !== false) {
    state.nextCategoryIndex = (currentIndex + 1) % state.categoryOrder.length;
  }
  state.lastBatch = Object.assign({}, batch, {
    completedAt: new Date().toISOString(),
    categoryProcessedAfter: state.categoryHistory[batch.categoryId].length,
    pointerAdvanced: options.advancePointer !== false,
