...
function selectVariantsByTier(variants, { fallbackUrl = null } = {}) {
  ...
  for (const variant of variants || []) {
    ...
    const candidate = {
      ...
      exactStandard: normalized.rawWidth === Number(normalized.resolution.split('x')[0]) &&
        normalized.rawHeight === Number(normalized.resolution.split('x')[1]),
    };
    const existing = byTier.get(normalized.quality);
    const better = !existing ||
      (candidate.exactStandard && !existing.exactStandard) ||
      (candidate.exactStandard === existing.exactStandard && candidate.bandwidth > existing.bandwidth) ||
      (candidate.exactStandard === existing.exactStandard && candidate.bandwidth === existing.bandwidth &&
        candidate.exactVariant && !existing.exactVariant);
...
function streamsFromManifest(server, text, manifestUrl, { verified = false } = {}) {
  return qualitiesFromManifest(text, manifestUrl).map((variant) => ({
    ...
    exactStandard: variant.exactStandard,
    verified,
  }));
}
