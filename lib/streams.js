...
function normalizeStreamEntry(entry) {
  ...
  return {
    ...
    exactStandard: entry.exactStandard === true ||
      (normalized.rawWidth && normalized.rawHeight && entry.resolution === normalized.resolution),
  };
}
...
function preferenceOf(entry) {
  return (entry.verified ? 8 : 0) + (entry.exactStandard ? 4 : 0) + (entry.exactVariant ? 2 : 0);
}
