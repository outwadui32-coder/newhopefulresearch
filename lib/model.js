...
function buildCategoryModel({ category, lastUpdated = null, purpose = '', items = [] } = {}) {
  ...
  for (const item of items) {
    ...
    // Keep aired episode metadata even when every provider is temporarily
    // unavailable. Writers emit no URL for an empty server list, so the three
    // formats still agree without inventing an unverified stream.
